...
LD_FLAGS += -L/usr/lib -Wl,-rpath=/usr/lib
LD_FLAGS += -lvart -lvart-dpu-controller
LD_FLAGS += -lglog -lvart -lvart-dpu-controller
LD_FLAGS += -lprotobuf
...
LD_FLAGS += -L=/usr/lib
LD_FLAGS += -lvitis_ai_library-dpu_task -lvitis_ai_library-xnnpp -lvitis_ai_library-model_config -lvitis_ai_library-math
...
demo_hfnet: $(OUTPUT_DIR)/demo_hfnet.o $(OUTPUT_DIR)/hfnet.o
	$(CXX) -v -std=c++17 $(CXX_FLAGS) -I.  -o $@ $+ $(LD_FLAGS)  -lvart-util
