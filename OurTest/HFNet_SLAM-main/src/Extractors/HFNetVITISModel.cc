#include "Extractors/HFNetVITISModel.h"
...
HFNetVITISModel::HFNetVITISModel(const std::string &strModelDir, ModelDetectionMode mode, const cv::Vec4i inputShape)
{
    mStrTRModelDir = strModelDir + "/";
    mMode = mode;
    // mInputShape = {inputShape(0), inputShape(1), inputShape(2), inputShape(3)}; // define input shape
    mStrONNXFile = mStrTRModelDir + "HF-Net.onnx";
    mStrCacheFile = mStrTRModelDir + "HF-Net.cache";

    // mbVaild = LoadHFNetTRModel();
    // load the DPU model and create the runtime ========

    if (!mbVaild) return;

    // mpBuffers.reset(new BufferManager(mEngine)); // BufferManager is not supported
...
bool HFNetVITISModel::Detect(const cv::Mat &image, std::vector<cv::KeyPoint> &vKeyPoints, cv::Mat &localDescriptors, cv::Mat &globalDescriptors,
                            int nKeypointsNum, float threshold)
{
    // if (mMode != kImageToLocalAndGlobal && mMode != kImageToLocalAndIntermediate) return false;

    // // Mat2Tensor(image, mvInputTensors[0]); //change accordingly
...
bool HFNetVITISModel::Detect(const cv::Mat &image, std::vector<cv::KeyPoint> &vKeyPoints, cv::Mat &localDescriptors,
                            int nKeypointsNum, float threshold)
{
...
bool HFNetVITISModel::Detect(const cv::Mat &intermediate, cv::Mat &globalDescriptors)
{
...
/*
// void HFNetVITISModel::GetLocalFeaturesFromTensor(const RTTensor &tScoreDense, const RTTensor &tDescriptorsMap,
...
// void HFNetVITISModel::GetGlobalDescriptorFromTensor(const RTTensor &tDescriptors, cv::Mat &globalDescriptors)
// {
...
// bool HFNetVITISModel::LoadHFNetTRModel(void)
// {
...
// void HFNetVITISModel::LoadTimingCacheFile(const std::string& strFileName, std::unique_ptr<IBuilderConfig>& config, std::unique_ptr<ITimingCache>& timingCache)
// {
...
// void HFNetVITISModel::UpdateTimingCacheFile(const std::string& strFileName, std::unique_ptr<IBuilderConfig>& config, std::unique_ptr<ITimingCache>& timingCache)
// {
...
// std::string HFNetVITISModel::DecideEigenFileName(const std::string& strEngineSaveDir, ModelDetectionMode mode, const Dims4 inputShape)
// {
//     string strFileName;
...
// bool HFNetVITISModel::SaveEngineToFile(const std::string& strEngineSaveFile, const unique_ptr<IHostMemory>& serializedEngine)
// {
...
// bool HFNetVITISModel::LoadEngineFromFile(const std::string& strEngineSaveFile)
// {
...
// void HFNetVITISModel::PrintInputAndOutputsInfo(unique_ptr<INetworkDefinition>& network)
// {
...
// void HFNetVITISModel::Mat2Tensor(const cv::Mat &mat, RTTensor &tensor)
// {
//     cv::Mat fromMat(mat.rows, mat.cols, CV_32FC(mat.channels()), static_cast<float*>(tensor.data));
//     mat.convertTo(fromMat, CV_32F);
// }

// void HFNetVITISModel::Tensor2Mat(const RTTensor &tensor, cv::Mat &mat)
// {
...
// void HFNetVITISModel::ResamplerRT(const RTTensor &data, const cv::Mat &warp, cv::Mat &output)
// {
